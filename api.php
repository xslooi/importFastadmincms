...
//    来源数据
    ...
    $from_db->query(" SELECT id, reid AS pId, typename AS name FROM `#@__arctype` ORDER BY sortrank ASC, id ASC ");
...
function act_start_v2($data) {
    ...
    $fromAddonTableFields = array();
    foreach($fromAddonTable as $item){
        $fromAddonTableFields[] = $item['Field'];
    }

    // 导入数据表附加表模型
    $toAddonTable = getChannelToAddonTable($import_typeid['to_typeid'], $TO_DB);
//var_dump($toAddonTable);
    $toAddonTableFields = array();
    foreach($toAddonTable as $item){
        $toAddonTableFields[] = $item['Field'];
    }

//    var_dump($toAddonTableFields);
      // 导入数据模型
    $dedecms_addon_table_desc = config('dedecms_addon_table_desc');

    $from_arcType = getArcType($import_typeid['from_typeid'], $FROM_DB);
    // var_dump($from_arcType);
    // todo 其他自定义模型处理
    if(6 < $from_arcType['channeltype']){
        $from_channeltype = 0;
        $msg =  '栏目【[' . $import_typeid['from_typeid'] . ']' . $from_arcType['typename'] . '】模型字段不匹配，请自行在Fastadmin-cms系统中新增模型并匹配字段后导入！';
    }
    elseif(-8 > $from_arcType['channeltype']){
        $from_channeltype = -100;
        $msg =  '栏目【[' . $import_typeid['from_typeid'] . ']' . $from_arcType['typename'] . '】模型字段不匹配，请自行在Fastadmin-cms系统中新增模型并匹配字段后导入！';
    }
    else{ // 系统默认模型
        $from_channeltype = $from_arcType['channeltype'];
        $msg =  '栏目【[' . $import_typeid['from_typeid'] . ']' . $from_arcType['typename'] . '】默认模型有改动，请自行在Fastadmin-cms系统中匹配字段后导入！';
    }

    // 判断是否存在默认字段数据
    if(!isset($dedecms_addon_table_desc[$from_channeltype])){
        return array(
            'id' => 6,
            'state' => -1,
            'msg' => '导入栏目模型没有默认字段信息请使用【act=generate_dedecms_addon】自行匹配生成或更改【dedecms_addon_table_desc.php】配置文件！',
            'data' => $from_channeltype
        );
    }

    $diff_table_desc = array();
    $dedecms_addon_table_default_fields = array();
    foreach($dedecms_addon_table_desc[$from_channeltype] as $item){
        $dedecms_addon_table_default_fields[] = $item['Field'];
    }

    foreach($fromAddonTable as $item){
        if(!in_array($item['Field'], $dedecms_addon_table_default_fields)){
            $diff_table_desc[] = $item;
        }
    }

    // 判断附加表多余字段是否在fastadmin-cms系统中
    // TODO 系统默认冲突字段处理
    $diff_table_filed_flag = false;
    foreach ($diff_table_desc as $item){
        if(!in_array($item['Field'], $toAddonTableFields)){
            $diff_table_filed_flag = true;
            break;
        }
    }

//        var_dump($diff_table_desc);
    if(!empty($diff_table_desc) && $diff_table_filed_flag){
        return array(
            'id' => 6,
            'state' => -1,
            'msg' => $msg,
            'data' => $diff_table_desc
        );
    }
...
function getFromData($tid, $db){
    ...
    if(-8 > $from_channelType['id']){
        $fromQuery = " SELECT addon.* FROM " . $from_channelType['addtable'] . " AS addon WHERE addon.typeid = {$tid} AND arcrank > -1 ";
    }
...
function insertFastadminOne($fromResults, $totype, $db){
    ...
    // 标志处理
    $flag = '';
    if(false !== strpos($fromResults['flag'], 'c')){
        $flag .= 'recommend';
    }

    if(false !== strpos($fromResults['flag'], 'h')){
        if(empty($flag)){
            $flag .= 'top';
        }
        else{
    ...
    // 跳转处理
    $outlink = '';
    if(false !== strpos($fromResults['flag'], 'j')){
        $outlink = $fromResults['redirecturl'];
    }

    $diyname = 'archive-' . $fromResults['id'];
    $fromResults['title'] = htmlspecialchars_decode($fromResults['title']);

    // 描述为空处理
    if(empty($fromResults['description']) && isset($fromResults['body']) && 10 < strlen($fromResults['body'])){
...
// 插入文章附加表
    $fromResults['content'] = $fromResults['body']; // 附加表固定字段赋值
...
//    echo "<hr>" . $query;
    ...
    if(!$intiny)
    {
        $db->query(" DELETE FROM `#@__cms_archives` WHERE id='$arcID' ");
